...
import { savePendingTransaction } from "@/lib/db/transactionDB";
...
export default function DashboardPage() {
    ...
    (cat.produk ?? []).map((p: any) => {
      if (cat.kategori === "fisik") {
        return {
          ...
          wholesalePrice:
            p.pricing?.grosir?.[0]?.hargaJual ||
            p.pricing?.eceran?.hargaJual ||
            0,
          stockRaw: p.stock || [],
          stock:
            p.stock?.reduce(
              (acc: number, cur: any) => acc + (cur.stok || 0),
              0,
            ) || 0,
        };
      }
  ...
  // ── Pending offline transactions sync ───────────────────────────────────
  const {
    pendingCount,
  ...
  // Badge sukses sync — tampil 5 detik lalu menghilang otomatis
  const [showSyncSuccess, setShowSyncSuccess] = useState(false);
  useEffect(() => {
    if (lastSyncResult && lastSyncResult.synced > 0 && pendingCount === 0) {
      setShowSyncSuccess(true);
      const t = setTimeout(() => setShowSyncSuccess(false), 5000);
  ...
  const addToCart = (product: any) => {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { product, qty: 1, pricingMode: "retail", harga: product.retailPrice },
      ];
    });
  };
  ...
  const setDigitalHarga = (id: number, harga: number) => {
    setCart((prev) =>
      prev.map((item) => (item.product.id === id ? { ...item, harga } : item)),
    );
  };
  ...
  const subtotal = cart.reduce((sum, item) => {
    if (item.product.category === "Produk Digital") {
      return (
        sum + ((item.harga || 0) + (item.product.biayaAdmin || 0)) * item.qty
      );
    }
    const price =
  ...
  // Helper: reset form setelah transaksi selesai
  const resetAfterTransaction = (delay = 1500) => {
    setTimeout(() => {
      setCart([]);
  ...
  const processTransaction = async () => {
    ...
    // Body request — identik dengan apa yang diterima backend
    const body = {
      customer: customerName.trim() || undefined,
      paymentType,
      paid: paymentType === "cash" ? Number(paymentAmount) : undefined,
      items: cart.map((item) => {
        ...
        const activeBatch =
          stockBatches.find((b: any) => b.stok >= item.qty) || stockBatches[0];
    ...
    // ── Mode offline: browser tahu tidak ada koneksi ─────────────────────────
    if (!navigator.onLine) {
      ...
      setTrxSuccess(
        "Tidak ada koneksi. Transaksi disimpan offline dan akan dikirim otomatis saat online.",
      );
      resetAfterTransaction(2200);
      setIsProcessing(false);
      return;
    }

    // ── Mode online: coba kirim ke server ────────────────────────────────────
    try {
      ...
      if (res.ok && data.success) {
        setTrxSuccess("Transaksi berhasil!");
        resetAfterTransaction(1500);
      } else {
        setTrxError(data.message || "Gagal memproses transaksi.");
      }
    } catch (err) {
      // Network error saat online — simpan ke antrian offline
      const isNetErr =
      ...
      if (isNetErr) {
        ...
        await refreshPendingCount();
        setTrxSuccess(
          "Koneksi terputus. Transaksi disimpan offline dan akan dikirim otomatis saat online.",
        );
        resetAfterTransaction(2200);
      } else {
        setTrxError("Terjadi kesalahan. Silakan coba lagi.");
      }
    } finally {
      setIsProcessing(false);
    }
  ...
  const cartContent = (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* Cart Items Area */}
      <div
        className={`flex-1 flex flex-col ${cart.length === 0 ? "items-center justify-center p-6 text-center bg-[#fdfdfd]" : "overflow-y-auto custom-scrollbar p-4"}`}
      >
        {cart.length === 0 ? (
          <>
            <IconShoppingCart
              ...
              stroke={1}
            />
            <p className="text-zinc-500 text-[15px] mb-1">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div className="flex justify-between items-start mb-1">
                    <div className="flex-1 pr-2">
                      <h4 className="text-[14px] font-medium text-zinc-900 line-clamp-1 mb-1.5">
                        {item.product.name}
                      </h4>
                        ...
                        <div className="relative inline-block w-full max-w-[130px]">
                          <select
                            ...
                            onChange={(e) =>
                              setPricingMode(
                                item.product.id,
                                e.target.value as "retail" | "wholesale",
                              )
                            }
                    ...
                    <button
                      ...
                      className="text-zinc-400 hover:text-red-500 transition-colors shrink-0 pt-0.5"
                    >
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <p className="text-[13px] font-bold text-zinc-900">
                      {item.product.category === "Produk Digital"
                        ? formatRupiah(
                            (item.harga || 0) + (item.product.biayaAdmin || 0),
                          )
                        : formatRupiah(
                            item.pricingMode === "retail"
                              ? item.product.retailPrice
      ...
      <div className="flex-1 flex flex-col items-center justify-center bg-[#f8fafc] p-4 text-center">
        <div className="max-w-md w-full bg-white border border-zinc-200/80 rounded-2xl p-8 shadow-sm flex flex-col items-center">
          ...
          <p className="text-sm text-zinc-500 mb-8 leading-relaxed">
            Anda harus memasukkan saldo awal kasir (uang modal) untuk memulai
            transaksi hari ini agar keuangan tercatat dengan rapi.
          </p>
          <a
            href="/keuangan"
    ...
    <div className="flex-1 flex overflow-hidden bg-[#f8fafc]">
      ...
      <div className="flex-1 flex flex-col p-4 lg:p-6 overflow-hidden relative">
        ...
        <div className="mb-3 shrink-0 space-y-2">
          ...
          {/* Pending Transactions — badge tanpa tombol, semua otomatis */}
          {(isSyncingPending || pendingCount > 0 || showSyncSuccess) && (
            <div className="flex items-center gap-2">
              ...
              {!isSyncingPending && pendingCount > 0 && (
              ...
              {showSyncSuccess && lastSyncResult && (
                <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-green-50 border border-green-200 text-green-700">
                  <IconCloudCheck className="w-3 h-3" stroke={2} />
                  {lastSyncResult.synced} transaksi tersinkronkan
                  {lastSyncResult.failed > 0 &&
                    ` · ${lastSyncResult.failed} gagal`}
                </span>
              )}
            </div>
          )}
                ...
                {filteredProducts.map((p) => (
                  ...
                  >
                    <div className="h-[120px] lg:h-[140px] bg-[#f4f4f5] flex items-center justify-center p-4 group-hover:bg-zinc-100 transition-colors">
                      <IconPackage
                        className="w-10 h-10 lg:w-12 lg:h-12 text-[#b07b54]"
                        strokeWidth={1.5}
                      />
                    </div>
                    <div className="p-3 lg:p-4 flex-1 flex flex-col justify-between">
                      <div>
                        <h3 className="font-medium text-zinc-900 text-[14px] lg:text-[15px] mb-1">
                          {p.name}
                        </h3>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          <Badge
                            variant={
                              p.type === "sendiri" ? "default" : "secondary"
                            }
                          ...
                          <span className="text-[11px] text-zinc-500 bg-zinc-100 px-1.5 rounded flex items-center">
                            {p.category === "Produk Fisik"
                              ? `Stok: ${p.stock}`
                              : "Digital"}
                          </span>
                        </div>
                      </div>
                      <p className="font-bold text-zinc-900 text-[14px] lg:text-base">
                ...
                {filteredProducts.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <h3 className="font-medium text-zinc-900 text-[15px]">
                        {p.name}
                      </h3>
            ...
            {paymentType === "cash" && (
              <div className="space-y-2">
                ...
                <Input
                  type="text"
                  value={
                    paymentAmount ? formatRupiah(Number(paymentAmount)) : ""
                  }
                ...
                {paymentAmount && Number(paymentAmount) >= total && (
                  <div className="p-3 bg-green-50 border border-green-100 rounded-lg flex items-center gap-2">
                    ...
                    />
                    <div>
                      <p className="text-[11px] text-green-600 font-medium leading-none mb-1">
                        Kembalian
                      </p>
                      <p className="text-sm font-bold text-green-700">
                        {formatRupiah(Number(paymentAmount) - total)}
                      </p>
                    </div>
                  </div>
                )}
